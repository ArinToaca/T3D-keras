...
# there is a minimum number of frames that the network must have, values below
# 10 gives -- ValueError: Negative dimension size caused by subtracting 3 from
# 2 for 'conv3d_7/convolution'
...
def train():
    ...
    video_train_generator = video_gen(
        d_train, FRAMES_PER_VIDEO, FRAME_HEIGHT, FRAME_WIDTH, FRAME_CHANNEL,
        nb_classes, batch_size=BATCH_SIZE)
    video_val_generator = video_gen(
        d_valid, FRAMES_PER_VIDEO, FRAME_HEIGHT, FRAME_WIDTH, FRAME_CHANNEL,
        nb_classes, batch_size=BATCH_SIZE)
...
if __name__ == '__main__':
    try:
        ...
        # Destroying the current TF graph to avoid clutter from old
        # models / layers
        K.clear_session()
