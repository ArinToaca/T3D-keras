...
from keras.layers import AveragePooling3D, MaxPooling3D, Dense
...
def _DenseLayer(prev_layer, growth_rate, bn_size, drop_rate):
    ...
    x = Activation('relu')(x)
    x = Conv3D(filters=bn_size * growth_rate, kernel_size=1, strides=1,
               padding='same')(x)
    x = Dropout(drop_rate)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Conv3D(filters=growth_rate, kernel_size=3, strides=1,
               padding='same')(x)
    x = Dropout(drop_rate)(x)
...
def _Transition(prev_layer, num_output_features):
    ...
    x = BatchNormalization()(prev_layer)
    x = Activation('relu')(x)
    x = Conv3D(filters=num_output_features, kernel_size=1, strides=1,
...
def _TTL(prev_layer):
    ...
    b1 = Conv3D(128, kernel_size=(1, 3, 3), strides=1, use_bias=False,
                padding='same')(b1)

    b2 = BatchNormalization()(prev_layer)
    b2 = Activation('relu')(b2)
    b2 = Conv3D(128, kernel_size=(3, 3, 3), strides=1, use_bias=False,
                padding='same')(b2)

    b3 = BatchNormalization()(prev_layer)
    b3 = Activation('relu')(b3)
    b3 = Conv3D(128, kernel_size=(4, 3, 3), strides=1, use_bias=False,
                padding='same')(b3)
...
def DenseNet3D(input_shape, growth_rate=32, block_config=(6, 12, 24, 16),
    ...
    pretrained_densenet = densenet.DenseNet169(
        include_top=False, input_shape=(224, 224, 3), input_tensor=inp_2d,
        weights='imagenet')
    ...
    y = GlobalAveragePooling2D(name='avg_pool_densnet2d')(
...
def densenet161_3D_DropOut(input_shape, nb_classes):
    model = DenseNet3D(input_shape, growth_rate=48, block_config=(
        6, 12, 36, 24), num_init_features=96, drop_rate=0.6,
...
def densenet121_3D_DropOut(input_shape, nb_classes):
    ...
    model = DenseNet3D(input_shape, num_init_features=64, growth_rate=32,
                       ...
                       drop_rate=0.6, num_classes=nb_classes)
